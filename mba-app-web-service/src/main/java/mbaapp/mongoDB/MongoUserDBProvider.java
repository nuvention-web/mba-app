...
//            schoolInfoJSON.put("MedianGMAT", info.getMedianGMAT());
    ...
    public void generateRecommendation(User user) throws Exception {
        ...
        Chunk headingChunk = new Chunk(user.getName() + " - MBA Application Profile", headingFont);

        Paragraph paragraph = new Paragraph(headingChunk);
        ...
        if(user.getWhyMBA()!=null && !user.getWhyMBA().isEmpty()) {
            addToDocument(document, "Firstly, why are you interested in pursuing an MBA at this point of your career?", user.getWhyMBA());
        }

        if(user.getShortTermGoals()!=null && !user.getShortTermGoals().isEmpty()){
            addToDocument(document, "What are your short term goals, following the completion of your MBA?", user.getShortTermGoals());
        }

        if(user.getLongTermGoals()!=null && !user.getLongTermGoals().isEmpty()){
            addToDocument(document, "What are your long term goals, following the completion of your MBA?", user.getLongTermGoals());
        }

        if(user.getLeadershipExperience()!=null && !user.getLeadershipExperience().isEmpty()){
            addBulletsToDocument(document, "List some of your experiences that demonstrate your leadership capabilities", user.getLeadershipExperience());
        }

        if(user.getTeamPlayerExperience()!=null && !user.getTeamPlayerExperience().isEmpty()){
            addBulletsToDocument(document, "List some of your experiences that demonstrate you are a team player and you work well in a collaborative environment", user.getTeamPlayerExperience());
        }

        if(user.getFailureExperience()!=null && !user.getFailureExperience().isEmpty()){
            addBulletsToDocument(document, "List some of your experiences where you failed you achieve what you set out to. What did you learn from these failures?", user.getFailureExperience());
        }

        if(user.getAccomplishments()!=null && !user.getAccomplishments().isEmpty()){
            addBulletsToDocument(document, "What are some of your accomplishments that you are most proud of, and why?", user.getAccomplishments());
        }

        if(user.getHobbiesOrInterests()!=null && !user.getHobbiesOrInterests().isEmpty()){
        ...
        Font imageFont = FontFactory.getFont(FontFactory.TIMES, 9, Font.ITALIC, BaseColor.BLACK);
        Chunk imageChunk = new Chunk("Generated by myapp.MBA.", imageFont);
        Paragraph imagePara = new Paragraph(imageChunk);
    ...
    private void addToDocument(Document document, String question, String answer) throws Exception{

        Font questionFont = FontFactory.getFont(FontFactory.TIMES, 11, Font.BOLD, BaseColor.BLACK);
        Chunk questionChunk = new Chunk(question, questionFont);

        Font answerFont = FontFactory.getFont(FontFactory.TIMES, 11, BaseColor.BLACK);
        Chunk answerChunk = new Chunk(answer, answerFont);

        Paragraph questionPara = new Paragraph(questionChunk);
        document.add(questionPara);

        Paragraph answerPara = new Paragraph(answerChunk);
        document.add(answerPara);
        document.add(Chunk.NEWLINE);
        //document.add(answerChunk);

    }

    private void addBulletsToDocument(Document document, String question, List<String> answers) throws Exception{
        ...
        Font answerFont = FontFactory.getFont(FontFactory.TIMES, 11, BaseColor.BLACK);
        Chunk answerChunk = new Chunk("", answerFont);
        Paragraph answerPara = new Paragraph(answerChunk);
        Font font = new Font();


        for(String answer:answers) {
            answerPara.add(bullet);
            answerPara.add(new Phrase(" " + answer + " ", font));
            answerPara.add(Chunk.NEWLINE);
        }
